...
const CartPage: React.FC<CartPageProps> = ({ cart, onUpdateQty, onRemove, offerDetails, applyCoupon, removeCoupon }) => {
  ...
  const handleProceed = async () => {
    for (const item of cart) {
      await firestoreHelpers.incrementWhatsApp(item.id);
    }
  ...
  if (cart.length === 0) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center p-8 text-center bg-gray-50">
        <div className="w-40 h-40 bg-white rounded-full flex items-center justify-center mb-8 shadow-xl">
          <ShoppingBag size={56} className="text-maroon/20" />
        </div>
        <h2 className="text-5xl font-serif text-gray-900 font-bold mb-4">Your treasure box is empty</h2>
        <p className="text-gray-400 mb-10 max-w-sm text-lg italic font-serif">A beautiful discovery is just a click away.</p>
        <Link to="/products" className="bg-maroon text-white px-12 py-5 rounded-full font-bold uppercase tracking-widest hover:bg-gold transition-all shadow-2xl">
          Explore Creations
        </Link>
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto px-4 py-16">
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 space-y-4">
          <div>
            ...
            <h1 className="text-6xl font-serif text-gray-900 font-bold">Your Selection</h1>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          <div className="lg:col-span-7 space-y-8">
            {cart.map(item => (
              <div key={item.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 flex items-center space-x-8 hover:shadow-md transition-all group">
                <div className="w-32 h-32 flex-shrink-0 bg-gray-50 rounded-2xl overflow-hidden group-hover:scale-105 transition-transform">
                  <img src={item.images[0]} alt={item.name} className="w-full h-full object-cover" />
                </div>
                <div className="flex-grow">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="text-xl font-serif text-gray-900 font-bold mb-1 leading-tight">{item.name}</h3>
                      <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{item.category}</p>
                    </div>
                    <button onClick={() => onRemove(item.id)} className="p-3 text-gray-300 hover:text-red-600 bg-gray-50 rounded-full transition-colors">
                      <Trash2 size={16} />
                    </button>
                  </div>
                  <div className="flex justify-between items-center mt-6">
                    <div className="flex items-center bg-gray-50 rounded-full p-1.5 border border-gray-100">
                      ...
                      <span className="mx-6 font-bold text-gray-900 w-6 text-center">{item.quantity}</span>
          ...
          <div className="lg:col-span-5">
            <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-gray-100 sticky top-24">
              <h3 className="text-3xl font-serif text-gray-900 font-bold mb-10">Grand Summary</h3>
              ...
              <div className="mb-10 pb-10 border-b border-gray-100">
                ...
                {offerDetails.activeCoupon ? (
                  <div className="flex items-center justify-between bg-green-50 px-6 py-4 rounded-2xl border border-green-100">
                    <div className="flex items-center space-x-3 text-green-700">
                      <Tag size={16} />
                      <span className="font-bold">{offerDetails.activeCoupon.code} Applied</span>
                    </div>
                  ...
                  <div className="flex space-x-2">
                    ...
                    <button 
                      onClick={handleApply}
                      className="px-8 bg-gray-900 text-white rounded-2xl font-bold uppercase tracking-widest text-[10px] hover:bg-black transition-all"
                    >
                      Apply
                    </button>
                  </div>
              ...
              <div className="space-y-5 mb-12">
                ...
                <div className="flex justify-between text-gray-900 font-bold text-3xl pt-8 border-t border-gray-100">
                  <span>Total</span>
              ...
              <button 
                ...
                className="w-full py-6 bg-maroon text-white rounded-[1.5rem] font-bold uppercase tracking-[0.2em] hover:bg-gold transition-all flex items-center justify-center space-x-3 shadow-2xl"
              >
                <Send size={20} />
                <span>Confirm on WhatsApp</span>
              </button>
              <p className="text-[10px] text-center text-gray-300 mt-6 leading-relaxed font-bold uppercase tracking-widest">
